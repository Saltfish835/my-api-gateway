...
public class GatewayCenterService {
    ...
    public ApplicationSystemRichInfo pullApplicationSystemRichInfo(String address, String gatewayId, String systemId) {
        ...
        String resultStr;
        try{
            resultStr = HttpUtil.post(address + "/wg/admin/config/queryApplicationSystemRichInfo", paramMap, 5000);
        }catch (Exception e) {
    ...
    public Map<String, String> queryRedisConfig(String address) {
        String resultStr;
        try {
            resultStr = HttpUtil.post(address + "/wg/admin/config/queryRedisConfig", "", 5000);
        }catch (Exception e) {
            logger.error("网关服务拉取Redis配置异常，连接资源不可用: {}", address + "/wg/admin/config/queryRedisConfig", e);
            throw e;
        }
